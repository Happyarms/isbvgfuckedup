...
(function () {
  ...
  function aggregateDisruptionsByType(departures) {
    ...
    for (var i = 0; i < departures.length; i++) {
      ...
      var product = departure.line.product;
      var isDelayed = !departure.cancelled &&
                      typeof departure.delay === 'number' &&
                      departure.delay > DELAY_THRESHOLD;
      var isCancelled = departure.cancelled === true;
  ...
  function updateTransitBoxes(aggregatedData) {
    ...
    var transitTypes = ['bus', 'ubahn', 'tram', 'sbahn'];

    // Update each transit type's delayed and cancelled counts
    for (var i = 0; i < transitTypes.length; i++) {
      var type = transitTypes[i];
