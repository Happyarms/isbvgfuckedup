...
- Production server deployed and running (see `DEPLOYMENT.md` in the root directory)
...
- **"npm: command not found"** - nvm not loaded:
  ```bash
  # Add to deploy.sh at the top:
  export NVM_DIR="$HOME/.nvm"
  [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"
  nvm use 18
  ```

- **"pm2: command not found"** - PM2 not installed or nvm not loaded (see above)
...
**Cause:** Application not running on port 3000 after deployment
...
# Should be a small number (seconds since last restart)
```

### External Verification

If your application is accessible via HTTPS:

```bash
# From your local machine
curl https://your-domain.com/api/status

# Check uptime to confirm recent deployment
curl -s https://your-domain.com/api/status | jq '.uptime'
```
...
- [ ] nvm and PM2 available in deployment script environment
...
- [ ] Application restarts successfully after deployment
- [ ] Deployment verification step passes (health check)
- [ ] No SSH permission errors
- [ ] No "command not found" errors (npm, pm2, git)

**End-to-End Pipeline:**
- [ ] Push to `main` triggers both CI and CD workflows
- [ ] Both workflows complete successfully
- [ ] Production server runs latest code after push
- [ ] Application uptime resets after deployment
- [ ] External health endpoint returns recent data
- [ ] No downtime during deployment (or minimal)
